...
$("#getCourseBtn").on("click", function () {
  $("#getCourseBtn").attr({ disabled: true });
  checkLearned();
});
...
function brush() {
  ...
  if (courseArr$[0]?.id) {
    ...
    let random = randomNumber(300, 1200, false).toFixed(
      randomNumber(2, 3, true)
    );
    console.log(
      `等待${(random / 60).toFixed(2)}分钟刷完`,
...
function addLog(log, random) {
  $.ajax({
    url: "http://www.tcm512.com/p_commoncall.do",
    type: "post",
    dataType: "json",
    data: {
      pn: "p_xxpt2022_editPlaySchedule",
      pv:
        courseId +
        "^^" +
        cid +
    ...
    success: function (data) {
      if (!random) {
        console.log(
          `${courseArr$[0].title}添加5%进度`,
          `${moment().format("YYYY-MM-DD HH:mm:ss")}`
        );
      } else {
        console.log(
          `${courseArr$[0].title}这节课刷完了`,
          `${moment().format("YYYY-MM-DD HH:mm:ss")}`
        );
        changeValue(2, cid);
        loadlist();
